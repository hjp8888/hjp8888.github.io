...
위키에만 있고 docs에 없는 티들러는 **전체 내보내기 목록의 공통 태그를 분석**해서 폴더 구조를 자동 결정해요.

공통 접두사가 2개 이상이면 하이픈, 1개면 일반 폴더예요.

```
[예시 A] 해운대·영도만 있을 때 — 공통 접두사 2개(대한민국, 부산) → 하이픈
  태그: "대한민국 부산 해운대" → docs/대한민국-부산/해운대/파일.md
  태그: "대한민국 부산 영도"   → docs/대한민국-부산/영도/파일.md

[예시 B] 부산·서울만 있을 때 — 공통 접두사 1개(대한민국) → 일반 폴더
  태그: "대한민국 부산" → docs/대한민국/부산/파일.md
  태그: "대한민국 서울" → docs/대한민국/서울/파일.md

[예시 C] 해운대·영도·서울 혼합 — 공통 접두사 1개(대한민국) → 일반 폴더
  태그: "대한민국 부산 해운대" → docs/대한민국/부산/해운대/파일.md
  태그: "대한민국 부산 영도"   → docs/대한민국/부산/영도/파일.md
  태그: "대한민국 서울"        → docs/대한민국/서울/파일.md
```

> 내보낸 파일의 frontmatter tags는 빈 문자열로 저장돼요.
> 폴더 구조 자체가 태그 역할을 하기 때문이에요.
...
build.py 실행
 ...
 │       ├── 전체 목록의 공통 태그 접두사 분석
 │       ├── 공통 접두사 2개 이상 → 하이픈 폴더 (대한민국-부산/)
 │       ├── 공통 접두사 1개 → 일반 폴더 (대한민국/)
 │       └── .md 파일로 내보내기 (frontmatter tags는 빈 문자열)
